...
explanation: ARD relevance gives a per-feature importance and the encoder metric $W^\top W$ gives
...
are the explanation, read off with no surrogate or permutation (Figure~\ref{fig:interpret}). The
ARD relevance $s_j$ is the inverse length scale of feature $j$; with the spectral energy the model
places on that feature it gives the importance
\begin{equation}
I_j\ \propto\ s_j^2\sum_{h} w_h\sum_{k} a_{h,j,k}^2\,\omega_{h,j,k}^2,
\label{eq:ardimp}
\end{equation}
the bank-weighted mean-square sensitivity of the embedding to $x_j$, a derivative-based global
index comparable across features because the inputs are standardized. On California the importance
