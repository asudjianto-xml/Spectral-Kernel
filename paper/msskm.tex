...
\title{\textbf{Multi-Scale Spectral Kernel Machines for Tabular Data}}
...
kernel is fixed the estimator is determined, so learning is the choice of a data-adapted
kernel. We develop the Multi-Scale Spectral Kernel Machine (MS-SKM), a structured and
inspectable kernel for tabular data, and we build it as a ladder in which each rung lifts
exactly one modeling restriction. The base rung is a generalized additive model from fixed
random Fourier features. Learning the frequencies keeps the model additive. Adding a radial
base kernel on the spectral embedding turns interactions on; we show these interactions come
from the kernel nonlinearity, not from feature mixing, and that a linear mixer can neither
create nor remove them---it only rotates them from axis-aligned to oblique. The top rung
partitions frequencies into multi-resolution banks, each with its own bandwidth, convex-fused
on the simplex. We prove the fused kernel is positive-semidefinite and that its fit decomposes
into one component per bank in the sum reproducing-kernel Hilbert space; we show that a single
linear mixer over banks is identical to one bank with the pooled frequencies, so the convex
fuse is the only construction that keeps the per-bank bandwidth, which is what makes multiple
banks more than more frequencies. A capacity accounting predicts, and a sweep confirms, that
the per-band frequency count is the overfitting axis while the number of banks is the
multi-scale axis. The ladder isolates each effect on real data: on California Housing almost
all of the gain over the additive model is the kernel and axis-aligned, on Breast Cancer it is
the oblique mixing, and multiple banks help only where multi-scale structure exists. A
variational head trains the same kernel by the ELBO of a sparse Gaussian process, so the model
also returns a calibrated predictive distribution.
\end{abstract}
...
\citep{yang2015}. Our contribution is an \emph{arrangement} of decisions into a tabular
inductive bias, and a \emph{ladder} that makes the contribution of each decision measurable.
...
interactions at all, and if so whether they are axis-aligned or oblique, and whether it needs
multiple scales or only more frequencies. Lifting one restriction at a time answers each
question separately, on the same data and the same training machinery.
...
and a per-coordinate relevance scale $s_j>0$, and define the feature map with components
\begin{equation}
...
$\mu_j=\sum_k a_{j,k}^2(\delta_{+s_j\omega_{j,k}}+\delta_{-s_j\omega_{j,k}})/2$. Hence $\psi$
realizes a sum of per-coordinate spectral-mixture kernels and is PSD. As $K\to\infty$ with
...
expected Gaussian-process behavior. The variational head trades a little accuracy for the
distribution --- its sparse $M$-point approximation reaches $R^2\approx0.84$ against the exact
decode's $0.88$, the standard inducing-point tradeoff --- and the same bound is the route to a
Bernoulli or softmax likelihood for classification, left for future work.
...
both crescents, and CatBoost draws an axis-aligned staircase. Accuracy sits at the noise floor,
so the metrics (Table~\ref{tab:moons}) read through the boundary: AUC saturates, but the kernel
rungs win on F1 and the Brier score, and CatBoost --- comparable on AUC --- has markedly worse
Brier and log loss, the signature of an accurate ranker whose probabilities are overconfident.
The variational head (Sec.~\ref{sec:elbo}) turns the same kernel into a predictive distribution
...
frequencies. \emph{[Sweep running; grids inserted on completion.]} The expected pattern, from
Prop.~\ref{prop:capacity} and Remark~\ref{rem:capacity}: rows (fixed $H$, increasing $K$) are flat
then decline as the ungated encoder overfits, columns (fixed $K$, increasing $H$) improve on
multi-scale data and are flat or worse on small data, and the best corner is moderate $K$ with
larger $H$.

\begin{table}[t]\centering
\caption{Test metric over $H$ (banks) $\times$ $K$ (frequencies per feature). \emph{To be filled
from} \texttt{benchmarks/sweep\_hk.py}.}
\label{tab:sweep}
\begin{tabular}{lccc}
\toprule
$H\backslash K$ & 8 & 16 & 32 \\
\midrule
1 & --- & --- & --- \\
2 & --- & --- & --- \\
4 & --- & --- & --- \\
...
kernel, not the mixer; the mixer only rotates them. Multiple banks are more than more frequencies
only because the convex fuse keeps a bandwidth per scale; a linear mixer over banks is one wider
bank. The per-band frequency count is the overfitting axis and the bank count is the multi-scale
axis. These are not asymptotic claims but statements we can read off real data by lifting one
restriction at a time. The open directions are a broader multi-seed benchmark against gradient
boosting, selecting the fusion weights by an unbiased risk estimate to inherit the oracle
guarantee, and a low-rank input projection before the per-coordinate map for genuinely
